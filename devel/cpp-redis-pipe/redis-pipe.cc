   ...
   try {
      ...
      for (;;) {
         ...
         try {
            ...
            // make redis requests
            client.incrby("upcdn:bw_chunk", bytes, cb); // Cumulative chunk bandwidth
            for (int expires = ts + 1; expires <= ts + 30; ++expires) { // Rolling 30s bandwidth
               char buf[sizeof "upcdn:bw_30sec_exp_+2147483648"];
               const std::string key = (std::sprintf(buf, "upcdn:bw_30sec_exp_%d", expires), buf);
               client.incrby(key, bytes, cb).expireat(key, expires, cb);
            }
            if( status == 404 && domain == "$UPCDN_HOSTNAME" ) {
               client.hsetnx("upcdn:404_uris", uri, "1");
            }
            // commit requests
